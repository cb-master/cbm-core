...
class Image
{
    ...
    public function crop(int $x, int $y, int $width, int $height): static
    {
        ...
        imagecopyresampled($cropped, $this->image, 0, 0, $x, $y, $width, $height, $width, $height);
        ...
        $crop = imagecrop($this->image, ['x' => $x, 'y' => $y, 'width' => $width, 'height' => $height]);
        if ($crop !== false) {
            $this->image = $crop;
            $this->width = $width;
            $this->height = $height;
    ...
    public function thumbnail(int $width, int $height, string $mode = 'fit'): static
    {
        ...
        if(!in_array($mode, ['fit','cover'])) throw new InvalidArgumentException("Unsupported Mode: '{$mode}'");
    ...
    public function save(string $path, int $quality = 100): bool
    {
        ...
        return match ($this->mime) {
            ...
            'image/png'               => imagepng($this->image, $path, min(9, (int)((100 - $quality) / 10))),
